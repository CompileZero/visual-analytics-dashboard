...
class GermanyData {
  ...
  loadDailyData = async (setRegions) => {
    ...
    await this.getCovidDataCases(
      "https://api.corona-zahlen.org/districts/history/cases/10"
    );
    await this.getCovidDataDeaths(
      "https://api.corona-zahlen.org/districts/history/deaths/10"
    );
    ...
    let max_cases = Math.max.apply(
      Math,
      arrCovidData.map(function (o) {
        return o.cases;
      })
    );
    ...
    for (let i = 0; i < this.mapRegions.length; i++) {
      ...
      const covidDataRegion = arrCovidData.find((region) => {
        if (
          region.name.normalize("NFD").replace(/\p{Diacritic}/gu, "") ===
            mapDataRegion.properties.NAME_3.normalize("NFD").replace(
              /\p{Diacritic}/gu,
              ""
            ) ||
          region.name
          ...
          mapDataRegion.properties.NAME_3.normalize("NFD")
            .replace(/\p{Diacritic}/gu, "")
            .includes(
              region.name.normalize("NFD").replace(/\p{Diacritic}/gu, ""),
              0
      ...
      if (covidDataRegion) {
        ...
        if (covidDataRegionCases) {
          this.mapRegions[i].properties.history_cases =
            covidDataRegionCases.history;
        }

        if (covidDataRegionDeaths) {
          this.mapRegions[i].properties.history_deaths =
            covidDataRegionDeaths.history;
        }
  ...
  loadHistoryData = async (setRegionsHistory) => {
    await this.getCovidData(
      "https://api.corona-zahlen.org/districts/history/deaths/7"
    );
